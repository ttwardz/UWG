...
extension Node where Context == HTML.BodyContext {
    static func tagList(for tags: [Tag], on site: UWG) -> Node {
    ...
    static func tagList(for item: Item<UWG>, on site: UWG) -> Node {
        return .tagList(for: item.tags, on: site)
        
    }
    
    static func tagList(for page: TagListPage, on site: UWG) -> Node {
